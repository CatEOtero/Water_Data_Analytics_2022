...
> aggregation: smooth over data gaps by aggregating i.e. if don't have daily data for every month but have same for each month then could use a central tendencey like mean to aggregate up and then do the analysis on. Good reason to do: some time series requires no temporal autocorrelation. monthly data may be less autocorrelated than daily data. This is why time-series isn't used that often with daily data. lake smapling best practice is every two weeks or monthly to go out into field and check. 

> subsampling: a lot of approachs require you to train your model on some of your data and then run the model on the rest of it as the analysis. another example is if have daily data then you could sample on first of every week

> interpolation: use surrounding data to fill in the gaps, time-series don't let you run with NAs

Specific tests for monotonic trend analysis are listed below, with assumptions and tips: 

* **linear regression**: no seasonality, fits the assumptions of a parametric (conform to a normal distribution) test. Function: `lm`
...
# generate total monthly discharge, multiplying by end thing to get data to be acre ft per month
...
```{r}
#someone added a jitter on top so you can see the data on top 
Condplot <-
ggplot(NeuseCond_monthly, aes(x = as.factor(Month), y = Conductivity)) +
  geom_boxplot() +
  labs(x = Month, y = expression("Conductivity ("*mu*"S/cm)")) #micrometers per centimeters

Flowplot <-
ggplot(NeuseFlow_monthly, aes(x = as.factor(Month), y = Discharge_acftmo)) +
  geom_boxplot() +
  labs(x = Month, y = Discharge (acre-ft))

plot_grid(Condplot, Flowplot, ncol = 1, align = "hv") #aligns horizontally and vertically, can filter out the months b/c top graph has NAs so has 13 ticks instead of 12 for months, could do ggplot(subset(NeuseCond_monthly, !is.na(Month)), aes....rest is the same as above)
```
...
# positive z score is that it is a positive trend overtime, S tracks the trends overtime, bigger z score (whether positive or negative) then p-value is smaller. The summary gives you a monthly report of s, p and z scores. remember what month/whenever you decided to start because may come up with different name i.e. may say season one and have to know that's October in this case
```

What would we conclude based on these findings? 

>More sig effect in the winter months
...
# run the arima function and search for best fit, arima checks the errors and tells you which one is the best option, we can use this to define out autocorrelation and forecasting
...
# make a prediction into the future, 10 years of 12 months each
Neuseprediction <- predict(fit, n.ahead = 10*12)

# plot future predictions
ts.plot(NeuseFlow_timeseries, Neuseprediction$pred, lty = c(1, 3))

#the prediction is bad haha so much random component that the forecast is bad. This is not the correct model (the autoregressive, moving average model) for this data
```

How do future predictions compare to the past? Why might the forecast be lacking in complexity?

> Less complex b/c don't have the data to feed into it directly. only data on one side of the "gap" If think of it like interpolating you only have the past points to interprolate with so less complex. 
