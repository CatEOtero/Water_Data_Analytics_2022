...
```{r}
# Join data frames to add state name onto the geo dataset b/c don't know what the state id in LAGOSgeo of the top of head
...
LAGOSMN <- LAGOSjoined %>% 
  ...
  filter(Month == 6:9) %>% 
  filter(Year >- 1990) %>% 
  filter(hu4 == "0401", "0701", "0702", "0703", "0704", "0902")

#8 log10 often used in stressor relationships to get a linear relationship
LAGOSMN_plot <- ggplot(LAGOSMN, aes()) +

#to include hu4 as an interaction term (aka interaction effect) we use the multiplication symbol. the plus symbol would have just given us one slope for all and an intercept for each different one.
