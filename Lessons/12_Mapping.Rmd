...
NOTE for EDA: the flowline df we were supposed to get out of this has a flowlength in km 
...
Lots of warnings that are thrown using nhdplus but as long as getting the functionality just keep chugging
...
```{r}
# Recall there are a lot of parameters measured at the site. 1 failed to parse message is okay.
NeuseParams <- whatNWISdata(siteNumbers = "02089500")

# Extract latitude and longitude for the site, need to do this for dam EDA sites, may need to download more data or change geometry into lat and long if you can back do this.
...
#start_comid will show up as a value not a df in the environment
#below are other ways to run the starting point
# start_point2 <- st_as_sf(data.frame(x = NeuseCoords$dec_long_va, y =  NeuseCoords$dec_lat_va), 
#                             coords = c("x", "y"), crs = 4269)

# Navigate the NLDI network, has flowlines
NLDI <- navigate_nldi(list(featureSource = "comid", featureID = start_comid), 
                          mode = "upstreamTributaries", 
                          distance_km = 1000)

# Extract watershed and flowpath information, set up a temp file to extract stuff, currently the info is a list and we can extract stuff from the list and turn it into a df

subset_file <- tempfile(fileext = ".gpkg")
subset <- subset_nhdplus(comids = as.integer(NLDI$UT$nhdplus_comid),
                         output_file = subset_file,
                         nhdplus_data = "download", #download the nhdplus data associated with the ID
                         flowline_only = FALSE, #get more info than just the flowline from the download
                         return_data = TRUE, overwrite = TRUE)
#This step will take awhile probs

# Create data frames, how get points
...
# find gages near watershed, find other gages in the area upstream of the current gage. Can also specify the distance of the search by putting in a length-look at help or the warning for help
gages <- get_nwis(AOI = catchment)
class(gages)

# find gages only within watershed, Kateri's most used tool in this package, find a subset of gages that are actually within the catchment and not just in the area found above, keep just the gages within the catchment
gages <- st_intersection(gages, catchment)

#there's an opposite of intersection it's difference. Look at the sf cheatsheet. https://www.rstudio.com/resources/cheatsheets/ go here and search simple features (sf)
```
...
```{r}
#can specify the minimum stream order you want to show i.e. 3 here, if don't specify streamorder than it defaults to 1.
#this code is not dependent on the previous code an independent visualization
...
#tricky because above eno is tiny area
ggplot(nc) +
  geom_sf(fill = "white") +
...
#catchment area is bigger and more spatial data, lines, pts, and polygons
ggplot(catchment) +
...
If we want to know specific info about catchment then we can do stuff like below.
1. How are flowlines distributed with respect to stream order? I.e. how many streamlines are what sream order. Neuse up to 6 and Eno up to 4. 
...
ggplot(subset(flowline, slope >= 0),
  ...
  geom_boxplot() +
  scale_y_log10()
#higher order have lower slope because flatter, gain voulme as river gets larger and tend to slow down and tend to get flatter. also rivers tend to go from high steep ground to low flat ground. tend to see this across rivers and streams.

#this is in total sq km of the specific catchment of the flowline and tracks the upstream and downstream nodes, so what is total area of upstream nodes (all those nodes above the outlet). This seems useful and simple  
